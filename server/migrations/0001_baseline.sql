-- Baseline schema, capturing what the old runtime `initSchema()` used to
-- create on every cold start. Written idempotently (IF NOT EXISTS) so it is
-- safe to run against an already-populated dev or production database that
-- was previously bootstrapped the old way.
