    ...
    <div className="flex flex-1 flex-col items-center justify-center px-4 py-20 text-center">
      ...
      {status === 'ok' && (
        <>
          ...
          <p className="mt-2 text-ink-500">Your provider email is now confirmed.</p>
        </>
      )}
      {status === 'error' && (
        <>
          ...
          <p className="mt-2 text-ink-500">Request a new verification email from your dashboard.</p>
        </>
      )}
      <Link to="/provider/dashboard" className="mt-6 rounded-full bg-brand-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-600">
        Go to dashboard
      </Link>
    </div>
  )
